...
class ControllerService implements ControllerServiceInterface
{
    ...
    {
        $this->moduleDir = ucfirst($moduleDir) . '/src/Controller';
        ...
        $this->classNameSpace = 'Pacificnm\\' . $this->moduleName . '\Controller';
        
        $this->factoryNamespace = 'Pacificnm\\' . $this->moduleName . '\Controller\Factory';
    ...
    public function createConsole()
    {
        ...
        $this->code->addUse('Pacificnm\\' . $this->moduleName . '\Service\ServiceInterface');
        ...
        $this->factory->addUse('Pacificnm\\' . $this->moduleName . '\Controller\ConsoleController');
        
        $dockBlock = new DocBlockGenerator();
        
        $dockBlock->setTags(array(
            new ParamTag('serviceLocator', 'ServiceLocatorInterface'),
            new ReturnTag('\Pacificnm\\' . $this->moduleName . '\Controller\ConsoleContorller')
        ));
    ...
    protected function getIndexDocBlock()
    {
        ...
        $dockBlock->setLongDescription('@see \Pacificnm\Controller\AbstractApplicationController::indexAction()');
    ...
    protected function getFormUse()
    {
        ...
        $this->code->addUse('Pacificnm\Controller\AbstractApplicationController');

        $this->code->addUse('Pacificnm\\' . $this->moduleName . '\Service\ServiceInterface');
        
        $this->code->addUse('Pacificnm\\' . $this->moduleName . '\Form\Form');
    ...
    protected function getUpdateBody()
    {
        ...
        $moduleName = strtolower($filter->filter($this->moduleName));
        ...
        $body .= "\t\t" . '$' . lcfirst($this->moduleName) . 'Entity = $this->service->save($entity);' . "\n\n";
        
        $body .= "\t\t" . '$this->getEventManager()->trigger(\'' . $moduleName . '-update\', $this, array(' . "\n";
        $body .= "\t\t\t" . '\'authId\' => $this->identity()->getAuthId(),' . "\n";
        $body .= "\t\t\t" . '\'requestUrl\' => $this->getRequest()->getUri(),' . "\n";
        $body .= "\t\t\t" . '\'' . lcfirst($this->moduleName) . 'Entity\' => $entity' . "\n";
        $body .= "\t\t" . '));' . "\n\n";
        
        $body .= "\t\t" . '$this->flashMessenger()->addSuccessMessage(\'Object was saved\');' . "\n\n";
        
        $body .= "\t\t" . 'return $this->redirect()->toRoute(\'' . $moduleName . '-view\', array(' . "\n";
        $body .= "\t\t\t" . '\'id\' => $' . lcfirst($this->moduleName) . 'Entity->get' . $this->moduleName . 'Id()' . "\n";
        $body .= "\t\t" . '));' . "\n";
        $body .= "\t" . '}' . "\n";
        $body .= '}' . "\n\n";
    ...
    protected function createFactory($name, $filename, $controller, $includeForm = false)
    {
        ...
        $this->factory->addUse('Pacificnm\\' . $this->moduleName . '\Controller\\' . $controller);
        
        $dockBlock = new DocBlockGenerator();
        
        $dockBlock->setTags(array(
            new ParamTag('serviceLocator', 'ServiceLocatorInterface'),
            new ReturnTag('\Pacificnm\\' . $this->moduleName . '\Controller\\' . $controller)
        ));
        
        $body = '$realServiceLocator = $serviceLocator->getServiceLocator();' . "\n\n";
        $body .= '$service = $realServiceLocator->get(\'Pacificnm\\' . $this->moduleName . '\Service\ServiceInterface\');' . "\n\n";
        
        if ($includeForm) {
            $body .= '$form = $realServiceLocator->get(\'Pacificnm\\' . $this->moduleName . '\Form\Form\');' . "\n\n";
    ...
    protected function writeClassFile($filename)
    {
        ...
        file_put_contents($this->moduleDir . '/' . $filename, $file->generate());
    }
    ...
    protected function writeFactoryFile($filename)
    {
        ...
        file_put_contents($this->moduleDir . '/Factory/' . $filename, $file->generate());
    }
}
